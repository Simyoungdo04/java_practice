...
public class UncheckedException {
	 ...
	 * ArithemticException : 나누기 연산을 0으로 나누면 발생하는 예외
	...
	public void divZero() {
		...
		*/
		 ...
		 * 
		 * try {
		 * 		(num1 / num2) 코드에서 예외가 발생할 수도 있는 구문
		 * } catch(발생할예외클래스명 변수명) {
	...
	public void code() {
		...
		while(true) {
			...
			try {
				...
				System.out.println("정수형 값만 입력해주세요.");
			}
			sc.nextLine();
	...
	public int multiCatch() {
		...
		*/
		try {
			...
			return 0;
		} finally { // return 직전 무조건 finally 블럭을 수행
			sc.close();
		}
	...
	public void orderByCatch() {
		...
		// ArrrayIndexOutOfBoundsException : 배열의 크기보다 큰 인덱스에 접근하면
		try {
			int size = sc.nextInt();
			...
			System.out.println("101 이상값만 입력해주세요");
		} catch(RuntimeException e) {
			// e.printStackTrace(); // 개발단계에서만 사용 | 공부할 수 있음
