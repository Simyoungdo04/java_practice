...
public class ServerProgram {
	...
	public static void main(String[] args) {
		 ...
		 * - 기반스트림만으로 한글 전소에 제약이 있어서 보조스트림을 사용
		 * 
		 * 소켓 통신용 서버측 프로그램 작성 절차
		 * 
		 * 1) 포트 번호 지정(서버에서 몇번 포트를 열것인지)
		 * 2) ServerSocket객체 생성 => 매개변수 생성자 호출 시 포트번호 인자로 전달(포트결함/Binding)
		 * 3) 대기상태
		 * 4) 클라이언트가 연결 요청을 하면 수락 후 통신이가능한 Socket 객체를 반환받음
		...
		// 자원반납을 위해 미리 참조변수 선언한고 null값으로 초기화
		...
		try {
			...
			// 바이트스트림과 문자스크림을 호환해 줄 수 있는 보조 스트림 => InputStreamReader
			// 속도향성, 한글전달을 위한 입력용 보조스트림 => BufferdReader => 2Byte
			...
			while(true) {
				...
				// 7_2) 출력하기
				System.out.print("클라이언테에게 전송할 메시지 > ");
