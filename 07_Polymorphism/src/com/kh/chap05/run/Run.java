...
public class Run {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < soups.length; i++) { // 반복횟수를 지정해야할때
			if(filter.test(soups[i])) {
				System.out.println(soups[i].getName());
			}
		}
		
		for(Soup s : soups) { // 배열의 처음부터 끝까지 돌리고 싶을때
			if(filter.test(s)) {
				System.out.println(s.getName());
			}
		}
		
		// 10000원 보다 비싼 김치찌개만 골라주세요
		...
		for(Soup s : soups) {
			if(shortNameSoupFilter.test(s)) {
				System.out.println("이름이 짧은 김치찌개 : " + s.getName());
			}
		}
		...
		// 3단계 : 매개변수의 자료형 제거 : 타입추론 + 매개변수가 한 개라면 괄호도 생략가능
