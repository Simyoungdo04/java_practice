...
public class MemberDao {
	...
	// Service단에서 필요할 때마다 Dao 기본생성자를 호출하고 있음
	...
	public boolean idCheck(Connection conn, String memberId) {
		try(PreparedStatement pstmt = conn.prepareStatement(prop.getProperty("idCheck"))){
	...
	public int singUp(Connection conn, MemberDto member) {
		int result = 0;
		try(PreparedStatement pstmt = conn.prepareStatement(prop.getProperty("singUp"))){
	...
	public List<BoardDto> selectBoardList(Connection conn) {
		...
		try(PreparedStatement pstmt = conn.prepareStatement(prop.getProperty("selectBoardList")); 
			ResultSet rset = pstmt.executeQuery()) {
			while(rset.next()) {
				BoardDto bd = new BoardDto(rset.getInt("BOARD_NO"), rset.getString("BOARD_TITLE"), null,
