...
public class ConditionSwitch {
	...
	public void login() {
		...
		// 3. 가입 시 입력했던 아이디와 비밀번호를 둘 다 정확히 입력했다면 로그인 성공화면 출력
		...
		// 만약에 사용자가 로그인 시 입력한 아이디 값와 회원가입 시 입력한 아이디 값이 동일 하고
		// 사용자가 로그인 시 입력한 패스워드 값과 회원가입시 입력한 패스워드 값이 동일하다면
		
		// 저장소 바꾸고 / 유효성검증 / 비밀번호암호화
		int count = 0;
		...
		// 실무에서는 권장하지 않는 방법
		// 아이디가 잘못입력 되면 아이디가 없다고 출력
		// 비밀번호가 잘못된거면 비밀번호가 틀렸다 출력
		// 보안에 좋지 않음
		
		// 기본자료형 : boolean char byte short int long float double
		// => 기본자료혀의 경우 동등비교연산자를 사용해서 비교연산을 수행함(==, !=)
